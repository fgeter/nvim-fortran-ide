...
| `<leader>sa` | Files containing ALL given words (any lines; `"word"` = whole word, `!word` = exclude; in picker `<C-s>`/`<C-a>` or `n`/`N` jump matches in preview) | `plugins/telescope.lua` |
