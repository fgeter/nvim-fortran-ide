...
| `hscrollbar.lua` | Horizontal + vertical scrollbars (strips at bottom / right of pane; click/drag). Under `laststatus<3` the horizontal bar is *compact*: the editor's own statusline row is the track (`___hscroll_track`) and the zero-height bar window below it renders that editor's mini.statusline (`___hscroll_status`, pre-rendered with `nvim_eval_statusline` in the parent's context), so the two rows read track-then-status with none wasted. Track clicks are hit-tested in `htrack_hit` (statusline row → `winrow == height + 1`, `line == 0`). Under `laststatus=3` it falls back to a 1-row split with the track as buffer content |
...
Shared project-runner logic lives in **`lua/core/project.lua`**: root resolution (`project.roots()`), executable discovery (`project.find_executables`, filtered by `vim.g.project_executable_pattern`, default `'*'`), the two-step executable→run-directory picker (`project.pick_and_launch` — lists the subdirectories of `vim.g.project_work_root` — most recently changed first, each labelled with its age, with a Sort entry that toggles to alphabetical for the session and `vim.g.project_workdir_sort` setting the initial order — plus a Browse entry that prompts for any other directory, with `<Tab>` path completion), pre-run output cleaning (`project.clean_output_files`, driven by `vim.g.project_clean_output_patterns`, no-op when unset; it always lists the matched file names and confirms before deleting any of them — the patterns are broad and the run dir can be any folder — defaulting to Yes under `project_work_root` and No elsewhere), and the build-terminal success suffix (`project.build_done_suffix`). `cmake-tools.lua`, `make-tools.lua`, and `fortran-tools.lua` all consume these helpers — none of them hardcodes paths or executable names.

Terminals for both come from **`lua/core/utils.lua`**, which opens one-shot bottom splits through a shared `one_shot_terminal` helper: `utils.run_build_cmd` closes the window as soon as the shell exits, while `utils.run_program_cmd` (used by the `<leader>cr` launchers) appends `; exit` so the shell quits with the program's status, then asks `Close the terminal?` with Yes as the default — answering No keeps the output on screen and maps `q` in that buffer to close it. `utils.make_terminal()` is the separate *persistent* shell, now used only for `make clean` in `make-tools.lua`.
