...
| `hscrollbar.lua` | Horizontal + vertical scrollbars (strips at bottom / right of pane; click/drag) |
...
Shared project-runner logic lives in **`lua/core/project.lua`**: root resolution (`project.roots()`), executable discovery (`project.find_executables`, filtered by `vim.g.project_executable_pattern`, default `'*'`), the two-step executable→run-directory picker (`project.pick_and_launch` — lists the subdirectories of `vim.g.project_work_root` plus a Browse entry that prompts for any other directory, with `<Tab>` path completion), pre-run output cleaning (`project.clean_output_files`, driven by `vim.g.project_clean_output_patterns`, no-op when unset; it always lists the matched file names and confirms before deleting any of them — the patterns are broad and the run dir can be any folder — defaulting to Yes under `project_work_root` and No elsewhere), and the build-terminal success suffix (`project.build_done_suffix`). `cmake-tools.lua`, `make-tools.lua`, and `fortran-tools.lua` all consume these helpers — none of them hardcodes paths or executable names.
